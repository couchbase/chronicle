...
get_options() ->
    Dispatch = cowboy_router:compile([
        {'_', [
            {"/config/info", ?MODULE, #state{domain=config, op=info}},
            {"/config/addnode", ?MODULE, #state{domain=config, op=addnode}},
            {"/config/provision", ?MODULE, #state{domain=config, op=provision}},
...
config_api(Req, #state{domain=config, op=addnode}) ->
    ...
    {ok, Vs} = chronicle:get_voters(),
    Message = case lists:member(Node, Vs) of
                  true ->
                      <<"already member">>;
                  _ ->
                      case net_kernel:connect_node(Node) of
                          true ->
                              ?DEBUG("nodes ~p", [nodes()]),
                              Result = chronicle:add_voters([Node]),
